...
exports.Login = async (req, res) =>{
    ...
    try{
        respRepo =await repoUserPg.pgsql_db.one('SELECT id,password FROM "User" WHERE email = $1',[req.body.email])
        if(!respRepo.err && respRepo.password == req.body.password){
            //Generate JWT Token
            const jwtToken =  jwt.sign({id:respRepo.id,email:req.body.email}, process.env.TOKEN_SECRET, { expiresIn: '1800m' });
           
            message = 'Success Response', data = {token:jwtToken}, statusCode = enum_.CODE_OK ;
            
        }else{
            status = 'Failure', message = "", statusCode = enum_.CODE_NOT_FOUND;
        }
        ...
        return res.status(enum_.CODE_NOT_FOUND).send(resp);
    }
}
